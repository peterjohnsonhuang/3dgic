...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, objects_folder, debug=True):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        gt_depth = None
        gt_depth_path = os.path.join(os.path.dirname(images_folder), "filtered/depths", os.path.basename(extr.name).replace(".png", ".tiff"))
        ...
        if "DSCF" in image_name:
            # print(os.path.basename(image_path))
            image_path = image_path.split('.')[0].removesuffix(os.path.basename(image_path.split('.')[0]))+'frame_{0:05d}.png'.format(idx+1)
            image_name_old = image_name
            image_name = os.path.basename(image_path).split(".")[0]
            # print(image_path)
            # input()
        ...
        # print(image_path)
        try:
            image = Image.open(image_path) if os.path.exists(image_path) else Image.open(image_path.split('.')[0]+'.png')
        ...
        try:
            ...
            id_name = '{0:05d}'.format(idx)
        except:
            id_name = image_name

        # print(id_name)
        
        object_path = os.path.join(objects_folder, id_name + '.png')
        if not os.path.exists(object_path):
            object_path = os.path.join(objects_folder, image_name + '.png')
        if not os.path.exists(object_path) and "DSCF" in image_name_old:
            object_path = os.path.join(objects_folder, image_name_old + '.png')
            # print(object_path)
        # print(object_path)
        # input()
        objects = Image.open(object_path).convert('L') if os.path.exists(object_path) else None
        # print(object_path)
        # print(objects)
        ...
        if os.path.basename(objects_folder) == 'object_mask':
            # print("suceed")
            depth_path = os.path.join(depth_folder, id_name + '_original.pt') if os.path.exists(os.path.join(depth_folder, id_name + '_original.pt')) else os.path.join(depth_folder, image_name + '_original.pt')
            depths = torch.load(depth_path).cpu() if os.path.exists(depth_path) else None
            # print(depth_path)
            # print(depth_path)
            ...
            if depths is not None:
                ...
                depth_id = os.path.basename(depth_path).split("_")[0]
                depth_remove = torch.load(os.path.join(depth_folder, depth_id+'_remove.pt')).cpu().squeeze()
                # print(depth_remove.shape)
            ...
            depth_path = os.path.join(depth_folder, id_name + '.png') if os.path.exists(os.path.join(depth_folder, id_name + '.png')) else os.path.join(depth_folder, image_name + '.png')
            # print(depth_path)
            depths = Image.open(depth_path) if os.path.exists(depth_path) else None
            depth_range_path = (depth_path.split('.')[0].split('_')[1]+'_range.pt') if os.path.exists(depth_path.split('.')[0].split('_')[1]+'_range.pt') else  os.path.join(depth_folder, id_name + '_range.pt')
            # print(depth_range_path)
            depth_range = torch.load(depth_range_path) if os.path.exists(depth_range_path) else None
        # print(depth_path)
        # print(id_name, image_name)
        gt_image_path = os.path.join(gt_folder, id_name + '.png') if os.path.exists(os.path.join(gt_folder, id_name + '.png')) else os.path.join(gt_folder, image_name + '.png')
        gt_image = Image.open(gt_image_path) if os.path.exists(gt_image_path) else None
        original_obj_path = os.path.join(original_obj_folder, id_name + '.png') if os.path.exists(os.path.join(original_obj_folder, id_name + '.png')) else os.path.join(original_obj_folder, image_name + '.png')
        original_obj = Image.open(original_obj_path) if os.path.exists(original_obj_path) else None
